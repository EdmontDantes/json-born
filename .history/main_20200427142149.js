...
&& process.argv[6] !== undefined && process.argv[7] !== undefined) {
    fs.writeFile('./users.json', function(error, data) {
        ...
        const userJsonWriteVar = {
            name: process.argv[5],
            age: process.argv[6],
            eyecolor: process.argv[7]
            
        }
        const userJsonWriteVarInternal = JSON.stringify(userJsonWriteVar);
        userJsonWriteVarInternal.push(userJsonWriteVar);

        // const userJsonCanIgetItBack = JSON.parse(data);
        console.log(userJsonWriteVarInternal);
    })
}